...
int ledPin3 = 6;  // Pin where the third LED is connected
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (received == '1') {
      ...
      digitalWrite(ledPin3, LOW);   // Ensure the third LED is off

      buzzerState = false; // Reset buzzer state
      ledState1 = false;   // Reset first LED state
      ledState2 = false;   // Reset second LED state
      ledState3 = false;   // Reset third LED state
    }
  }
